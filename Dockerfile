...
# ── ADIM 1: Temel araçları önce kur (curl burada kurulur) ───
...
# ── ADIM 2: Node.js 20 LTS repo ekle (curl artık mevcut) ───
RUN curl -fsSL https://deb.nodesource.com/setup_20.x | bash -

# ── ADIM 3: Tüm sistem paketleri ────────────────────────────
RUN apt-get update && apt-get install -y --no-install-recommends \
    \
    # X11
    xvfb \
    ...
    screen \
    \
    # Node.js 20 (repo eklendi, artık bu kurulur)
    nodejs \
    \
    # Network araçları
    net-tools \
